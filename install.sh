...
INSTALLER_VERSION="2026.07.07.19"
...
setup_nat() {
  ...
  if kox_fetch_to_file "99-kox-nat.sh" "${NAT_DIR}/99-kox-nat.sh" 15 \
      && [ -s "${NAT_DIR}/99-kox-nat.sh" ]; then
    chmod +x "${NAT_DIR}/99-kox-nat.sh"
    ok "NAT-скрипт загружен (${NAT_DIR}/99-kox-nat.sh)"
  elif [ -f "$(dirname "$0")/99-kox-nat.sh" ]; then
    cp "$(dirname "$0")/99-kox-nat.sh" "${NAT_DIR}/99-kox-nat.sh"
    chmod +x "${NAT_DIR}/99-kox-nat.sh"
    ok "NAT-скрипт скопирован"
  else
    warn "99-kox-nat.sh не найден на CDN — записываю встроенную версию"
