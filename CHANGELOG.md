...
### 🛠 UDP: только звонки и Clash Royale, игры напрямую

- TPROXY больше не на весь интернет-UDP — из‑за этого COD, РФ-серверы и другие
  игры не находили игроков на карте.
- В туннель по UDP только Telegram, WhatsApp (Facebook CIDR) и Supercell
  `5.180.72.0/22`. Clash Royale TCP 9339 как было.
- Остальные игры (CODM и т.д.) идут с домашнего IP, как до TPROXY.

## 2026.09.15.26

### 🛠 COD Mobile: «не удалось присоединиться к бою»

- Логин 443 шёл в VPN, матч UDP — напрямую: разные IP, сервер ждал и кикал.
- Категория `codm` снята с туннеля (логин и бой с одного домашнего IP).
- UDP 5056/8011/8080 тоже напрямую; QUIC Tencent не дропаем.

## 2026.09.15.25

### 🛠 COD Mobile: пустой лобби и таймаут

- Поиск игроков идёт UDP **5055 / 20002**, не только 7500–8000. Через HY2
  пакеты не доходят — лобби пустое, потом kick.
- Эти порты снова **напрямую** (как до TPROXY). Логин/CDN CODM по 443 в VPN.
- TCP 65010/65050 больше не перехватываем: лобби и матч должны быть с одного IP.

## 2026.09.15.24

### ✨ Call of Duty Mobile + бот на роутере отставал от CLI

- Категория **`codm`**: Activision / Tencent (логин, CDN, античит).
- iptables **TCP 65010 и 65050** (лобби и чат CODM) → Xray, как 9339 у Clash Royale.
- На Keenetic часто оставался старый `kox-bot` — из‑за этого бот писал
  «есть обновление», хотя `kox` уже был свежий.

## 2026.09.14.23

### ✨ Игры, которые в РФ не открываются + починка обновления списков

- Новые категории: **`roblox`** (блок РКН снят 10.06.2026, список опциональный),
  **`riot`**, **`blizzard`**, **`epic`**, **`minecraft`**.
...
- **Fix failover**: «текущий сервер» берётся из `KOX_SERVER`, не из `config.json` (HY2 больше не показывает `127.0.0.1`).
- **Failover статус**: корректные сообщения «на основном / на резервном / другой сервер».
- **Упрощённое меню**: настройки → failover + расширенные; инструменты компактнее.
- **🌐 Серверы**: кнопка **copy_text** — скопировать URL подписки; failover и update-sub здесь.
- **setMyCommands** обновлён (`/settings`, `/update`, `/sub`).

## 2026.07.07.18

### ✨ Telegram-бот: резервный сервер и полное управление

- **Failover**: выбор **резервного сервера** в боте (`⚙️ → VPN-failover → 🛡 Резервный`).
- **`kox switch-auto`**: при сбое сначала пробует заданный резервный, затем остальные из подписки.
- **Инструменты в боте**: Fix NAT, auto-failover, watchdog-лог, восстановление бэкапа, обновление подписки, clean legacy.
- **Домены**: удаление IP, загрузка всех категорий списков кнопкой.
- **Настройки**: проверка обновления KOX и скан legacy VPN из меню.
