...
# Реальный VPN-сервер из kox.conf (в HY2 config.json address = 127.0.0.1).
bot_current_server_host() {
  if [ -f "$KOXCONF" ]; then
    # shellcheck disable=SC1090
    . "$KOXCONF" 2>/dev/null
    [ -n "${KOX_SERVER:-}" ] && printf '%s' "$KOX_SERVER" && return 0
  fi
  grep -m1 '"address"' "$CONF" 2>/dev/null | sed 's/.*"address": *"\([^"]*\)".*/\1/'
}

bot_failover_status_line() {
  local cur pref back back_rm pref_rm line
  [ -f "$KOXCONF" ] && . "$KOXCONF" 2>/dev/null
  cur=$(bot_current_server_host)
  pref="${KOX_PREFERRED_HOST:-}"
  back="${KOX_BACKUP_HOST:-}"
  back_rm="${KOX_BACKUP_REMARK:-}"
  pref_rm="${KOX_PREFERRED_REMARK:-}"
  [ -z "$pref" ] && return 0
  if [ "$cur" = "$pref" ]; then
    line="✅ <b>Активен основной сервер</b>: ${pref_rm}"
  elif [ -n "$back" ] && [ "$cur" = "$back" ]; then
    line="⚠️ <b>Сейчас на резервном</b>: ${back_rm}
<code>${cur}</code>"
  elif [ "$cur" != "$pref" ]; then
    line="⚠️ <b>Активен другой сервер</b>: <code>${cur}</code>
(не основной ${pref_rm})"
  fi
  [ -n "$line" ] && printf '%s\n' "$line"
}
...
setup_commands() {
  ...
  CMDS=$(jq -cn '[
    ...
    {"command":"status",   "description":"📊 Статус VPN"},
    {"command":"settings", "description":"⚙️ Настройки и failover"},
    {"command":"on",       "description":"✅ Включить VPN"},
    {"command":"off",      "description":"❌ Выключить VPN"},
    {"command":"update",   "description":"⬆️ Обновить KOX Shield"},
    {"command":"sub",      "description":"🔗 URL подписки"},
...
domains_keyboard() {
  printf '%s' '{
    "inline_keyboard":[
      ...
      [{"text":"➖ Удалить IP","callback_data":"prompt_del_ip"},
       {"text":"📋 Категории списков","callback_data":"listcats"}],
  ...
  printf '%s' '{
    "inline_keyboard":[
      [{"text":"📝 Логи Xray","callback_data":"log"},
       {"text":"🐕 Watchdog лог","callback_data":"watchdog_log"}],
      [{"text":"🔧 Тест конфига","callback_data":"test_config"},
       {"text":"🔧 Fix NAT","callback_data":"do_fix_nat"}],
...
h_status() {
  local CHAT="$1"
  send_typing "$CHAT"
  local XRAY_OK PORT_OK IPT_OK VPN_ST SRV CONN ERRSIZE IPT_TCP
  ...
  SRV=$(bot_current_server_host)
  CONN=$(netstat -tn 2>/dev/null | grep ':10808' 2>/dev/null | wc -l | tr -d ' \n\r')
  case "$CONN" in ''|*[!0-9]*) CONN=0 ;; esac
  ERRSIZE=$(ls -lh "$ERRLOG" 2>/dev/null | awk '{print $5}' || echo "—")
  IPT_TCP=$(iptables -t nat -vL XRAY_REDIRECT 2>/dev/null | grep "REDIRECT.*tcp" | awk '{print $1" pkts"}' | head -1)
...
h_servers_menu() {
  ...
  if [ -n "$SUB_URL" ]; then
    KBD=$(jq -cn --arg sub "$SUB_URL" '{
      "inline_keyboard":[
        [{"text":"🔀 Сменить сервер","callback_data":"switch_list"}],
        [{"text":"🛡 VPN-failover","callback_data":"failover_settings"}],
        [{"text":"🔄 Обновить подписку","callback_data":"do_update_sub"}],
        [{"text":"📋 Скопировать URL подписки","copy_text":{"text":$sub}}],
        [{"text":"◀️ Главное меню","callback_data":"menu"}]
      ]
    }')
  else
...
<code>config_${TS}.json</code>" \
    "$(printf '{"inline_keyboard":[[{"text":"◀️ Бэкапы","callback_data":"backup_menu"}]]}')"
}
...
h_del_ip() {
  ...
  if grep -qF "\"${IP}\"" "$CONF" 2>/dev/null; then
    grep -vF "\"${IP}\"" "$CONF" > /tmp/kox-tmp.json && mv /tmp/kox-tmp.json "$CONF"
    "$XRAY_INIT" restart >/dev/null 2>&1
    update_menu "$CHAT" "✅ IP <code>${IP}</code> удалён, Xray перезапущен"
  else
    update_menu "$CHAT" "⚠️ IP <code>${IP}</code> не найден"
  fi
}
...
h_fix_nat() {
  local CHAT="$1"
  send_typing "$CHAT"
  OUT=$(/opt/bin/kox fix-nat 2>&1 | sed 's/\x1B\[[0-9;]*m//g' | tail -8)
  update_menu "$CHAT" "🔧 <b>Fix NAT (YouTube / QUIC)</b>

<pre>$(printf '%s' "$OUT")</pre>" "$(tools_back_keyboard)"
}

h_switch_auto() {
  local CHAT="$1"
  send_typing "$CHAT"
  update_menu "$CHAT" "⏳ <b>Auto-failover...</b>

Проверяю серверы из подписки (резервный — первым, если задан)."
  OUT=$(/opt/bin/kox switch-auto 2>&1 | sed 's/\x1B\[[0-9;]*m//g' | tail -15)
  update_menu "$CHAT" "🔀 <b>Auto-failover</b>

<pre>$(printf '%s' "$OUT")</pre>" \
    "$(printf '{"inline_keyboard":[[{"text":"◀️ Инструменты","callback_data":"tools_menu"}]]}')"
}

h_update_sub() {
  local CHAT="$1"
  send_typing "$CHAT"
  OUT=$(/opt/bin/kox update-sub 2>&1 | sed 's/\x1B\[[0-9;]*m//g' | tail -10)
  update_menu "$CHAT" "🔄 <b>Обновление подписки</b>
...
h_watchdog_log() {
  local CHAT="$1"
  local WDLOG="/opt/var/log/kox-watchdog.log"
  local TAIL
  TAIL=$(tail -25 "$WDLOG" 2>/dev/null || echo "(лог пуст)")
  update_menu "$CHAT" "🐕 <b>Watchdog log</b> (последние 25 строк)

<pre>$(printf '%s' "$TAIL" | sed 's/&/\&amp;/g; s/</\&lt;/g')</pre>" \
    "$(printf '{"inline_keyboard":[[{"text":"◀️ Инструменты","callback_data":"tools_menu"}]]}')"
}

h_restore_list() {
  local CHAT="$1"
  local BDIR="/opt/etc/xray/backups"
  printf '' > /tmp/kox-restore-list.txt
  ls -t "$BDIR"/config_*.json 2>/dev/null | head -5 | while IFS= read -r F; do
    [ -n "$F" ] && printf '%s\n' "$F" >> /tmp/kox-restore-list.txt
  done
  if [ ! -s /tmp/kox-restore-list.txt ]; then
    update_menu "$CHAT" "📂 <b>Бэкапы не найдены</b>
...
Сначала создайте: 💾 Бэкап" \
  ...
  while IFS= read -r F; do
    local LABEL
    LABEL=$(basename "$F" .json | sed 's/config_//')
    ROWS="${ROWS},[{\"text\":\"📂 ${LABEL}\",\"callback_data\":\"restore_${IDX}\"}]"
    IDX=$((IDX+1))
  done < /tmp/kox-restore-list.txt
  KBD="{\"inline_keyboard\":[${ROWS#,},[{\"text\":\"◀️ Бэкапы\",\"callback_data\":\"backup_menu\"}]]}"
  update_menu "$CHAT" "📂 <b>Восстановить конфиг из бэкапа</b>

Выберите файл (после восстановления Xray перезапустится):" "$KBD"
}

h_do_restore() {
  local CHAT="$1" IDX="$2"
  local F
  F=$(sed -n "$((IDX+1))p" /tmp/kox-restore-list.txt 2>/dev/null)
  [ -z "$F" ] && { update_menu "$CHAT" "❌ Бэкап не найден" "$(back_keyboard)"; return; }
  OUT=$(/opt/bin/kox restore "$F" 2>&1 | sed 's/\x1B\[[0-9;]*m//g' | tail -8)
  rm -f /tmp/kox-restore-list.txt
  update_menu "$CHAT" "📂 <b>Конфиг восстановлен</b>
...
<b>Меню кнопок:</b>
📊 Статус — Xray, VPN, сервер (реальный IP), NAT
🌐 Серверы — смена, failover, копировать URL подписки
✅ Вкл / ❌ Выкл / 🔄 Рестарт
📋 Домены — списки, категории, IP
🛠 Инструменты — логи, бэкапы, fix NAT
⚙️ Настройки — failover (⭐ основной + 🛡 резервный)

<b>Команды:</b> <code>/update</code> <code>/sub URL</code> <code>/settings</code>
...
🛡 <b>VPN-failover</b> — основной и резервный сервер
⚙️ <b>Расширенные</b> — автообновления и уведомления"

  KBD=$(jq -cn \
    '{"inline_keyboard":[
      [{"text":"🛡 VPN-failover →","callback_data":"failover_settings"}],
      [{"text":"⚙️ Расширенные →","callback_data":"settings_advanced"}],
      [{"text":"🧹 Проверить legacy VPN","callback_data":"clean_legacy"}],
      [{"text":"◀️ Главное меню","callback_data":"menu"}]
    ]}')
...
h_settings_advanced() {
  ...
  MSG="⚙️ <b>Расширенные настройки</b>

🔄 Автообновление KOX: ${ICON_UPG}
📋 Автообновление списков: ${ICON_LST}
${ICON_NUPG} Уведомления KOX / ${ICON_NLST} Уведомления списков"
...
h_backup_menu() {
  local CHAT="$1"
  update_menu "$CHAT" "💾 <b>Бэкапы конфигурации</b>

Сохраняют config.json и kox.conf перед экспериментами." \
    '{"inline_keyboard":[
      [{"text":"💾 Создать бэкап","callback_data":"do_backup"}],
      [{"text":"📂 Восстановить","callback_data":"restore_list"}],
      [{"text":"◀️ Инструменты","callback_data":"tools_menu"}]
    ]}'
}
...
h_failover_settings() {
  ...
  local BACK_REMARK="${KOX_BACKUP_REMARK:-не задан}"
  local BACK_HOST="${KOX_BACKUP_HOST:-}"
  local FMIN="${KOX_FAILOVER_MINUTES:-10}"
  local AUTORET="${KOX_AUTO_RETURN:-yes}"
  [ "$AUTORET" = "yes" ] && ICON_RET="✅ Вкл" || ICON_RET="❌ Выкл"

  local STATUS_LINE
  STATUS_LINE=$(bot_failover_status_line)
...
$([ -n "$PREF_HOST" ] && printf '<code>%s</code>\n' "$PREF_HOST")

🛡 <b>Резервный сервер</b>: ${BACK_REMARK}
$([ -n "$BACK_HOST" ] && printf '<code>%s</code>\n' "$BACK_HOST")
...
Когда основной сервер восстановится — вернуться на него."
  ...
  KBD=$(jq -cn \
    ...
    '{"inline_keyboard":[
      ...
      [{"text":"🛡 Выбрать резервный сервер","callback_data":"backup_pick_list"}],
      [{"text":"🗑 Сбросить резервный","callback_data":"clear_backup"}],
      [{"text":"🔀 Тест auto-failover","callback_data":"confirm_switch_auto"}],
...
h_failover_pick_list() {
  local CHAT="$1" MODE="$2"
  local TMP CB_PREFIX TITLE DESC MARK_SEL
  [ -f "$KOXCONF" ] && . "$KOXCONF" 2>/dev/null
  case "$MODE" in
    backup)
      TMP="/tmp/kox-backup-servers.txt"
      CB_PREFIX="set_backup_"
      TITLE="🛡 <b>Выберите резервный сервер</b>"
      DESC="При сбое VPN watchdog и auto-failover переключатся <b>сначала</b> на этот сервер."
      MARK_SEL="${KOX_BACKUP_HOST:-}"
      ;;
    *)
      TMP="/tmp/kox-pref-servers.txt"
      CB_PREFIX="set_pref_"
      TITLE="⭐ <b>Выберите основной сервер</b>"
      DESC="Это сервер, на который бот вернётся автоматически после восстановления связи."
      MARK_SEL="${KOX_PREFERRED_HOST:-}"
      MODE="pref"
      ;;
  esac
  ...
  printf '%s\n' "$DECODED" | grep -E "$KOX_URI_GREP" | while IFS= read -r VLINE; do
    ...
    printf '%s\t%s\t%s\t%s\n' "$IDX" "$HOST" "$PORT" "$ENC_REMARK" >> "$TMP"
    IDX=$((IDX+1))
  done

  [ ! -s "$TMP" ] && { update_menu "$CHAT" "❌ Серверы не найдены" "$(back_keyboard)"; return; }
  ...
  while IFS='	' read -r IDX HOST PORT ENC_REMARK; do
    ...
    [ -z "$REMARK" ] && REMARK="$HOST"
    [ "$HOST" = "$MARK_SEL" ] && MARK="✓ " || MARK=""
    ROWS="${ROWS},[{\"text\":\"${MARK}${REMARK}\",\"callback_data\":\"${CB_PREFIX}${IDX}\"}]"
  done < "$TMP"

  KBD="{\"inline_keyboard\":[${ROWS#,},[{\"text\":\"◀️ Назад\",\"callback_data\":\"failover_settings\"}]]}"
  update_menu "$CHAT" "${TITLE}

${DESC}" "$KBD"
}

h_pref_pick_list() {
  h_failover_pick_list "$1" pref
}

h_backup_pick_list() {
  h_failover_pick_list "$1" backup
}

h_set_preferred() {
  h_set_failover_server "$1" "$2" pref
}

h_set_backup() {
  h_set_failover_server "$1" "$2" backup
}

h_set_failover_server() {
  local CHAT="$1" IDX="$2" MODE="$3"
  local TMP KEY_HOST KEY_PORT KEY_REMARK ICON OK_TITLE
  case "$MODE" in
    backup)
      TMP="/tmp/kox-backup-servers.txt"
      KEY_HOST="KOX_BACKUP_HOST"; KEY_PORT="KOX_BACKUP_PORT"; KEY_REMARK="KOX_BACKUP_REMARK"
      ICON="🛡"; OK_TITLE="Резервный сервер установлен!"
      ;;
    *)
      TMP="/tmp/kox-pref-servers.txt"
      KEY_HOST="KOX_PREFERRED_HOST"; KEY_PORT="KOX_PREFERRED_PORT"; KEY_REMARK="KOX_PREFERRED_REMARK"
      ICON="⭐"; OK_TITLE="Основной сервер установлен!"
      MODE="pref"
      ;;
  esac

  [ ! -f "$TMP" ] && { h_failover_pick_list "$CHAT" "$MODE"; return; }
  local SRV_LINE
  SRV_LINE=$(grep "^${IDX}	" "$TMP" | head -1)
  ...
  [ -f "$KOXCONF" ] && . "$KOXCONF" 2>/dev/null
  if [ "$MODE" = "backup" ] && [ -n "${KOX_PREFERRED_HOST:-}" ] && [ "$HOST" = "$KOX_PREFERRED_HOST" ]; then
    update_menu "$CHAT" "⚠️ Резервный сервер не может совпадать с основным.

Выберите другой сервер." \
      "$(printf '{"inline_keyboard":[[{"text":"◀️ Назад","callback_data":"backup_pick_list"}]]}')"
    return
  fi
  if [ "$MODE" = "pref" ] && [ -n "${KOX_BACKUP_HOST:-}" ] && [ "$HOST" = "$KOX_BACKUP_HOST" ]; then
    update_menu "$CHAT" "⚠️ Основной сервер не может совпадать с резервным.

Выберите другой сервер." \
      "$(printf '{"inline_keyboard":[[{"text":"◀️ Назад","callback_data":"pref_pick_list"}]]}')"
    return
  fi

  conf_set "$KEY_HOST"   "$HOST"
  conf_set "$KEY_PORT"   "${PORT:-443}"
  conf_set "$KEY_REMARK" "$REMARK"
  rm -f "$TMP"

  update_menu "$CHAT" "${ICON} <b>${OK_TITLE}</b>
...
h_clear_backup() {
  local CHAT="$1"
  conf_set KOX_BACKUP_HOST ""
  conf_set KOX_BACKUP_PORT ""
  conf_set KOX_BACKUP_REMARK ""
  answer_cb "$CB_ID" "Резервный сброшен"
...
✓ — загружена на роутер"
  KBD='{"inline_keyboard":[
    [{"text":"⚡ Загрузить все категории","callback_data":"list_load_all","style":"success"}],
    [{"text":"◀️ Домены","callback_data":"domains_menu"}]
  ]}'
  update_menu "$CHAT" "📋 <b>Категории списков</b>

Список отправлен выше. Загрузить все — кнопкой ниже." "$KBD"
}

h_list_load_all() {
  local CHAT="$1"
  answer_cb "$CB_ID" "Загружаю списки..."
  update_menu "$CHAT" "⏳ <b>Загружаю все категории...</b>

Это может занять 1–2 минуты."
  OUT=$(/opt/bin/kox list-load all 2>&1 | sed 's/\x1B\[[0-9;]*m//g' | tail -12)
  update_menu "$CHAT" "✅ <b>Списки загружены</b>

<pre>$(printf '%s' "$OUT")</pre>" \
    "$(printf '{"inline_keyboard":[[{"text":"◀️ Домены","callback_data":"domains_menu"}]]}')"
}
...
Введите IP или CIDR (например: <code>1.2.3.0/24</code>):" \
        ...
        ;;
      prompt_del_ip)
        printf '%s' "del_ip|${CHAT_ID}" > "$WAIT_FILE"
        update_menu "$CHAT_ID" \
          "➖ <b>Удалить IP/подсеть</b>

Введите IP или CIDR для удаления:" \
          "$(back_keyboard)"
        ;;
      ...
      settings_advanced)     h_settings_advanced "$CHAT_ID" ;;
      backup_menu)           h_backup_menu       "$CHAT_ID" ;;
      failover_settings)     h_failover_settings "$CHAT_ID" ;;
      pref_pick_list)        h_pref_pick_list    "$CHAT_ID" ;;
      backup_pick_list)      h_backup_pick_list  "$CHAT_ID" ;;
      clear_backup)          h_clear_backup      "$CHAT_ID" ;;
      ...
      set_backup_*)
        BACK_IDX=$(printf '%s' "$CMD" | sed 's/set_backup_//')
        h_set_backup "$CHAT_ID" "$BACK_IDX" ;;
      set_ft_*)
...
Включить: <code>/settings</code> → Уведомления KOX" ;;
      ...
      list_load_all)       h_list_load_all "$CHAT_ID" ;;
...
Включить: <code>/settings</code>" ;;
      ...
      do_fix_nat)        h_fix_nat  "$CHAT_ID" ;;
      do_update_sub)     h_update_sub "$CHAT_ID" ;;
      watchdog_log)      h_watchdog_log "$CHAT_ID" ;;
      restore_list)      h_restore_list "$CHAT_ID" ;;
      confirm_switch_auto)
        update_menu "$CHAT_ID" \
          "⚠️ <b>Запустить auto-failover?</b>

Текущий сервер будет заменён на первый рабочий (резервный — если задан в настройках)." \
          "$(confirm_keyboard switch_auto)"
        ;;
      do_switch_auto)    h_switch_auto "$CHAT_ID" ;;
      restore_*)
        REST_IDX=$(printf '%s' "$CMD" | sed 's/restore_//')
        h_do_restore "$CHAT_ID" "$REST_IDX" ;;
      confirm_clearlog)
        update_menu "$CHAT_ID" \
