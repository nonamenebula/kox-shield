  ...
  printf '%s' '{
    "inline_keyboard":[
      ...
      [{"text":"🐕 Watchdog лог","callback_data":"watchdog_log"},
       {"text":"🔧 Fix NAT (YouTube)","callback_data":"do_fix_nat"}],
      [{"text":"🔀 Auto-failover","callback_data":"confirm_switch_auto"},
       {"text":"🔄 Обновить подписку","callback_data":"do_update_sub"}],
      [{"text":"💾 Бэкап","callback_data":"do_backup"},
       {"text":"📂 Восстановить бэкап","callback_data":"restore_list"}],
      [{"text":"🧹 Clean legacy","callback_data":"clean_legacy"},
...
Сначала создайте: 💾 Бэкап" \
  ...
  KBD="{\"inline_keyboard\":[${ROWS#,},[{\"text\":\"◀️ Инструменты\",\"callback_data\":\"tools_menu\"}]]}"
...
🛠 <b>Инструменты</b> — логи, бэкап, fix NAT, auto-failover, legacy
⚙️ <b>Настройки</b> — failover (основной + резервный), обновления

<b>Failover:</b>
⭐ Основной — куда вернуться после сбоя
🛡 Резервный — куда переключиться первым при падении VPN
...
Основной и резервный сервер, задержка, автовозврат"
  ...
  KBD=$(jq -cn \
    ...
    '{"inline_keyboard":[
      ...
      [{"text":"⬆️ Проверить обновление KOX","callback_data":"kox_do_upgrade","style":"primary"}],
